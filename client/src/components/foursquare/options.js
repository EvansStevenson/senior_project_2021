...
import { } from "../../actions/foursquareActions";
...
class CaesarOptions extends Component {
    ...
    key = e => {
        this.props.updateKey(e.target.value.toLowerCase(), this.props.foursquareExampleText)
    }

    plaintext = e => {
        this.props.updatePlaintext(e.target.value.toLowerCase(), this.props.foursquareExampleText)
    }
        ...
        return (
            <div>
                <div className="column" style = {{borderRight: "solid black 5px"}}>
                    <h1>Four-Square Cipher options</h1>  <Link to="/">Back To all Ciphers</Link>
                    <h3>About the cipher</h3>
                    <p></p>
                    <h3>How to use</h3>
                    <p></p>
                    <Link to="/foursquare/test"><button>Test yourself!</button></Link>
                </div>
                <div className="column">
                    ...
                    Plain text: <input id="plaintextInput" onChange={this.plainText}></input><button onClick={() => {
                        this.encrypt(); 
                        document.getElementById("cipherInput").value = this.props.foursquareExampleText.ciphertext;
                    }}>Encrypt</button><br/>
                    Cipher text: <input id="cipherInput" onChange={this.cipherText}></input><button onClick={() => {
                        this.decrypt();
                        document.getElementById("plaintextInput").value = this.props.foursquareExampleText.plaintext;
                    }}>Decrypt</button>
                </div>
            </div>
        )
